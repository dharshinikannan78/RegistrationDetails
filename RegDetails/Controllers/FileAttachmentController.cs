...
namespace RegDetails.Controllers
{
    ...
    public class FileAttachmentController : ControllerBase
    {
        ...
        private AttachmentModel SaveFileToDB(string fileName, string fileType, string filePathAttachment)
        {
            var objFiles = new AttachmentModel()
            {
                AttachmentId = 0,
        ...
        {
            var userData = dataModel.registerData.Where(a => a.CandidateId == candidateId)
                .FirstOrDefault();
